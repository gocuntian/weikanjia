...
if(isset($_REQUEST['wkjtype']) && $_REQUEST['wkjtype'] == 'jsapiPay'){
	$url = 'wkjv42WxpayApi.php?'.$_SERVER['QUERY_STRING'];
	header("Location:$url");
	exit;
}
...
else if( !empty($_SESSION['oid']) && !empty($_SESSION['apiurl']) )
{
	...
	$pageData['orderCallBackUrl']          = isset($datas['orderCallBackUrl']) ? $datas['orderCallBackUrl'] : '';	// 回调的地址
...
$res['orderid']      = $pageData['oid'];
$res['wxorderid']    = $pageData['wxorderid'];
$res['uid']          = $pageData['uid'];
$res['backUrl']      = $pageData['backurl'];
$res['wxSign']    = json_decode($jsApiParameters, true);
$res['wxpayQrUrl'] = $qrcode4pay;
$resJson             = json_encode($res);

$orderCallBackUrl = $pageData['orderCallBackUrl'].'&wxpaySign=' . urlencode($resJson);
header("Location:$orderCallBackUrl");
exit;
